...
# Redis connection
REDIS_HOST = 'sneakers-redis.omekpe.0001.use2.cache.amazonaws.com'
REDIS_PORT = 6379
...
def get_all_products():
    ...
    try:
        ...
        # Cache for 5 minutes
        cache_set('all_products', products)
        return products
    except Exception as e:
        print(f"Error: {e}")
        return []
...
def get_products_by_category(category):
    ...
    products = get_all_products()
    results = [p for p in products if p.get('category', '').lower() == category.lower()]
...
def get_products_by_brand(brand):
    ...
    products = get_all_products()
    results = [p for p in products if p.get('brand', '').lower() == brand.lower()]
