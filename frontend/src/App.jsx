                      ...
                      <div className="sneaker-details">
                        ...
                        <div className="product-actions">
                          ...
                          {(() => {
                            ...
                            return (
                              <button type="button" onClick={() => { const currentLowest = results[0].price; const raw = prompt('Enter target price for alert:', Math.floor(currentLowest * 0.9)); if (raw === null) return; const target = Number(raw); if (!Number.isFinite(target) || target <= 0 || target >= currentLowest) { window.alert(`Set a target below the current price ($${currentLowest}). You'll be emailed when it drops below your target.`); return; } addPriceAlert(sneakerInfo, target) }} className={`inline-flex items-center justify-center gap-2 rounded-lg border px-4 py-2.5 text-sm font-medium transition ${alertOn ? 'border-orange-500 bg-orange-500 text-[#2a1500]' : 'border-orange-500/50 bg-[#121212] text-white hover:border-orange-500 hover:bg-[#1c1c1c]'}`}>
